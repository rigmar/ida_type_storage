...
class print_ctree_action_handler_t(idaapi.action_handler_t):
    ...
    def activate(self, ctx):
        ...
        if vdui.item.is_citem():
            item =  vdui.item.it
            if item.op in [cot_memptr, cot_memref]:
                parent = vdui.cfunc.body.find_parent_of(item)
                if parent and parent.op == cot_call and parent.to_specific_type.x.op == cot_helper:
                    cast_helper = parent.to_specific_type.x.helper
                    helpers = ["BYTE", "HIWORD", "LOWORD"]
                    for h in helpers:
